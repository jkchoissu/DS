...
public:									// public functions
    Map(int capacity = 100);			// constructor
...
public:				// public types
    ...
    public:
      ...
      friend class Map;						// give Map access
    };
};

template <typename K, typename V>		// get entry
typename Map<K,V>::Entry& Map<K,V>::Iterator::operator*() const
    { return *ent; }

template <typename K, typename V>		// are iterators equal?
bool Map<K,V>::Iterator::operator==(const Iterator& p) const {
...
Map<K,V>::Map(int capacity) : n(0), B(capacity) { }

template <typename K, typename V>		// number of entries
int Map<K,V>::size() const { return n; }

template <typename K, typename V>		// is the map empty?
bool Map<K,V>::empty() const { return size() == 0; }

template <typename K, typename V>		// find utility
typename Map<K,V>::Iterator Map<K,V>::finder(const K& k) {
...
typename Map<K,V>::Iterator Map<K,V>::inserter(const Iterator& p, const Entry& e) {
...
void Map<K,V>::eraser(const Iterator& p) {
