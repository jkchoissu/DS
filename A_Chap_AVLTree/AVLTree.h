...
template <typename E>
class AVLTree {					// a binary search tree
public: 						// public types
...
typename AVLTree<E>::Iterator AVLTree<E>::end()
    { return Iterator(T.root()); }			// return the super root

template <typename E>						// find utility
typename AVLTree<E>::TPos AVLTree<E>::finder(const K& k, const TPos& v) {
...
typename AVLTree<E>::Iterator AVLTree<E>::find(const K& k) {
...
typename AVLTree<E>::TPos AVLTree<E>::inserter(const K& k, const V& x) {
  TPos v = finder(k, root());				// search from virtual root
...
typename AVLTree<E>::TPos AVLTree<E>::eraser(TPos& v) {
  TPos w;
...
template <typename E>
typename AVLTree<E>::TPos AVLTree<E>::restructure(const TPos& v) throw(BoundaryViolation) {
	TPos x = v;
    TPos y = x.parent();
    TPos z = y.parent();
...
typename AVLTree<E>::Iterator& AVLTree<E>::Iterator::operator++() {
  TPos w = v.right();
